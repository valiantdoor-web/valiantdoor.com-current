...
// Creating a LEAD (not a bare customer) is what triggers Housecall Pro's
// "New Lead" push/app notification. Lead creation requires a MAX plan.
...
async function forwardLeadToHousecall(fields, options = {}) {
  ...
  // The lead wraps a customer object; lead_source is set on both the customer
  // and the lead itself to match Housecall Pro's create-lead schema.
  const customer = {
    first_name,
    last_name,
