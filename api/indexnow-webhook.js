...
async function submitIndexNow({ reason, deploymentId, deploymentTarget }) {
  ...
  if (![200, 202].includes(response.statusCode)) {
    throw new Error(`IndexNow submission failed with ${response.statusCode}: ${response.body.slice(0, 300)}`);
  }
...
module.exports = async (req, res) => {
  ...
  if (expectedProjectId && projectId && expectedProjectId !== projectId) {
    sendJson(res, 202, { ignored: true, reason: 'Ignored different project', projectId });
    return;
  }

  if (target && target !== 'production') {
    sendJson(res, 202, { ignored: true, reason: 'Ignored non-production deployment', target });
    return;
  }
